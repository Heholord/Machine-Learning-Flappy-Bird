...
App.Main.prototype = {
	...
	create : function(){
		...
		this.GA = new GeneticAlgorithm(this.GA_MAX_UNIT, this.GA_BEST);
	...
	update : function(){
		switch(this.state){
			...
			case this.STATE_GAMEOVER: // when all birds are killed evolve the population
				this.GA.Population.forEach((pop) => {
					let logText = "Gen " + this.GA.iteration +  " species " + pop.index  + " fitness " + pop.fitness + " score " + pop.score;
					var xmlHttp = new XMLHttpRequest();
					let url = "http://localhost:3000?log="+btoa(logText);
